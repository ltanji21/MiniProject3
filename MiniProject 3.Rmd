...
Cleaning up the data via dplyr. Got rid of duplicate rows, movie ids, kind ids, etc. Arranged by year. 
```{r}
imdb_sequels <- imdb_sequels %>%
  group_by(production_year) %>%
  distinct(movie_id, .keep_all = TRUE) %>%
  select(production_year, title, link_type_id) %>%
  arrange(production_year)

glimpse(imdb_sequels)
```
